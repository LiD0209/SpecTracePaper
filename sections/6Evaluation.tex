...
For Table~\ref{tab:llm-comparison-implementation-full} and
Figure~\ref{fig:model-comparison-bars}, all non-model components are held
fixed: the same extracted rules, retrieval budgets, validators, adjudication
protocol, and test-agent settings are used for every model. Prompts differ only
in model-specific formatting constraints needed to obtain schema-valid JSON.
GPT-5.4 produces the largest number of confirmed fixes, while smaller or less
code-specialized models leave more reports pending or rejected. Provider-level
cost differences are analyzed separately in RQ4.
...
protocol. Figure~\ref{fig:model-cost-bars} compares the same stage-wise cost
breakdown across the four model families used in
Table~\ref{tab:llm-comparison-implementation-full}. Claude has the highest
estimated total cost, followed by GPT-5.4; DeepSeek and Qwen are about 35\%
lower than GPT-5.4.
...
\begin{figure}[!t]
\centering
% TODO: Replace these placeholder provider-cost values with measured billing
% logs before submission. Non-GPT stage values preserve the GPT-5.4 stage
% shares and scale to placeholder provider totals.
\begin{tikzpicture}
\begin{axis}[
  ybar,
  width=\columnwidth,
...
stage. The GPT-5.4 bars correspond to Table~\ref{tab:cost}; the remaining bars
are provider-level placeholder estimates that preserve the same stage shares.}
